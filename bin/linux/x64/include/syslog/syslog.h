...
#ifndef FRAMEX_SYSLOG_SYSLOG_H
#define FRAMEX_SYSLOG_SYSLOG_H
...
namespace framex {
	...
	class FRAMEX_SYSLOG_EXPIMP SysLog_D { // dynamic
	private:
		SysLog_D() {
		};
	...
	public:
		void SetDebugMode( bool debug_mode ); // 默认 true
		...
		void InitSysLog( std::string app_name, std::string app_version, std::string app_company, std::string app_copyright, std::string app_folder = "" ); // 设置好参数后再调用
		void PrintSysInfo();
		void WriteSysInfo();
	...
	class FRAMEX_SYSLOG_EXPIMP SysLog_S { // static
	private:
		SysLog_S() {
		};
