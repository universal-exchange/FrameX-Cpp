...
#ifndef FRAMEX_COMMON_ASSIST_H
#define FRAMEX_COMMON_ASSIST_H
...
namespace framex {
	...
	//	std::cout << "number 保留 " << i << " 位小数后为: " << framex::Round_1( number, i ) << std::endl; // Round_1、Round_2、Round_3
	//}
	...
	inline int64_t NearPrice( const double input_price, const int64_t min_price_unit ) { // 针对价格已经转换为整数形式
		int64_t input_price_temp = (int64_t)std::floor( input_price * 10 ); // 多保留一位，之后的截掉
		int64_t min_price_unit_temp = min_price_unit * 10; // 多保留一位
		int64_t remainder_temp = input_price_temp % min_price_unit_temp;
		int64_t min_price_unit_temp_half = min_price_unit_temp / 2; // 所得个位数必为 5
		// 下面比较大小，有可能均价刚好处于一个最小报价单位的中间，比如报价单位 0.05 均价 10.025 而上下两个价格为 10.05 和 10.00， 这里就用 >= 判断了，四舍五入嘛
		input_price_temp = remainder_temp >= min_price_unit_temp_half ? ( input_price_temp - remainder_temp + min_price_unit_temp ) : ( input_price_temp - remainder_temp );
		int64_t output_price = input_price_temp / 10;
		// std::cout << input_price << "：" << output_price << std::endl;
		return output_price;
	}
...
// C++17 已废弃 std::wstring_convert<std::codecvt_utf8<wchar_t>>
// 故使用下面 MultiByteToWideChar / WideCharToMultiByte 版的 GB2312toUTF8() 函数
//	inline std::string GB2312toUTF8( const std::string& source ) {
//#if defined(_MSC_VER)
...
//#endif
//		mbstate_t state = {};
//		wchar_t* wsz_next = nullptr;
//		const char* sz_next = nullptr;
//		std::vector<wchar_t> buff( source.size() );
//		int32_t res = std::use_facet<std::codecvt<wchar_t, char, mbstate_t>> (loc).in( state, source.data(), source.data() + source.size(), sz_next, buff.data(), buff.data() + buff.size(), wsz_next );
//		if( std::codecvt_base::ok == res ) {
//			std::wstring_convert<std::codecvt_utf8<wchar_t>> cvt_utf8;
//			return cvt_utf8.to_bytes( std::wstring( buff.data(), wsz_next ) );
//		}
//		return "";
//	}

// C++17 已废弃 std::wstring_convert<std::codecvt_utf8<wchar_t>>
// 故使用下面 MultiByteToWideChar / WideCharToMultiByte 版的 UTF8toGB2312() 函数
//	inline std::string UTF8toGB2312( const std::string& source ) {
//#if defined(_MSC_VER)
//		std::locale loc( "zh-CN" );
//#else
//		std::locale loc( "zh_CN.GB18030" );
...
//		const wchar_t* wsz_next = nullptr;
//		std::wstring_convert<std::codecvt_utf8<wchar_t>> cvt_utf8;
//		std::wstring wtemp = cvt_utf8.from_bytes( source );
//		std::vector<char> buff( wtemp.size() * 2 );
//		int32_t res = std::use_facet<std::codecvt<wchar_t, char, mbstate_t>> (loc).out( state, wtemp.data(), wtemp.data() + wtemp.size(), wsz_next, buff.data(), buff.data() + buff.size(), sz_next );
//		if( std::codecvt_base::ok == res ) {
...
//	}
	...
	inline void AnsiCharToWideChar( const char* source, std::wstring& result ) {
		...
		result = converter.from_bytes( source );
#endif
	}
...
#ifdef __OS_LINUX__
		std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> converter;
		result = converter.to_bytes( source );
#endif
	}
...
#ifdef __OS_LINUX__
    inline double GetPerformanceTime_Nano() { // 纳秒
	    double tick_time = 0.0;
		struct timespec time_spec;
	    // CLOCK_REALTIME：系统实时时间，随系统时间改变而改变，从 UTC 1970-01-01 00:00:00 开始
		// CLOCK_MONOTONIC：从系统启动开始计时，不受用户改变系统时间的影响
        ...
        if( 0 == clock_gettime( CLOCK_MONOTONIC, &time_spec ) ) {
            tick_time = time_spec.tv_sec + double( time_spec.tv_nsec ) / 1000000000.0;
		}
		return tick_time;
	}
#endif
...
#ifdef __OS_LINUX__
    inline double GetPerformanceTime_Micro() { // 毫秒
	    double tick_time = 0.0;
		struct timeval time_val;
		if( 0 == gettimeofday( &time_val, nullptr ) ) {
            tick_time = time_val.tv_sec + double( time_val.tv_usec ) / 1000000.0;
		}
		return tick_time;
	}
#endif
...
#ifdef __OS_LINUX__
  inline std::string GetIPAddressByHost() {
		char host_name[64] = { 0 };
		gethostname( host_name, sizeof( host_name ) );
		struct hostent* host_ent = gethostbyname( host_name );
		char ip_str[32] = { 0 };
		const char* result = inet_ntop( host_ent->h_addrtype, host_ent->h_addr_list[0], ip_str, sizeof( ip_str ) );
		if( nullptr == result ) {
			return "";
		}
