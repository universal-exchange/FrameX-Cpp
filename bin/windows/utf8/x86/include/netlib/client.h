...
#ifndef FRAMEX_NETLIB_CLIENT_H
#define FRAMEX_NETLIB_CLIENT_H
...
namespace framex {
	...
	struct NetClientInfo { // 用于网络事件推送
		...
		NetClientInfo( int32_t info_type, int32_t node_flag, std::string& node_type, int32_t identity, std::string& endpoint_l, std::string& endpoint_r );
	};
	...
	struct NetClientData { // 用于网络数据推送
		...
		NetClientData( int32_t node_flag, std::string& node_type, int32_t identity, int32_t code, int32_t func, std::string& data );
	};

	#pragma pack( pop )

	class FRAMEX_NETLIB_EXPIMP NetClient_X {
	public:
		NetClient_X();
		virtual ~NetClient_X();
	...
	class FRAMEX_NETLIB_EXPIMP NetClient {
	public:
		NetClient();
		~NetClient();
	...
	public:
		...
		bool Client_AddConnect( std::string address_r, int32_t port_r, int32_t node_flag_r, std::string node_type_r );
		void Client_SetAutoReconnect( bool auto_reconnect );

		int32_t Client_SendDataAll( int32_t type, int32_t code, int32_t func, std::string& data );
		int32_t Client_SendData( ConnectInfo* connect_info, int32_t type, int32_t code, int32_t func, std::string& data );
